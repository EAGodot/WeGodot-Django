...
class UpdateUserView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # 🔧 修復：簡化頭像返回邏輯
            avatar_a = ct.avatar if ct.avatar else ''  # 直接使用客戶端的avatar字段
            
            # 可選：檢查頭像文件是否存在（如果需要）
            # if ct.avatar and default_storage.exists(ct.avatar.replace('/media/', '')):
            #     avatar_a = ct.avatar
            # else:
            #     avatar_a = ''
...
class UpdateUserViewBackup(APIView):
    permission_classes = [AllowAny]
    authentication_classes = [TokenAuthentication]
